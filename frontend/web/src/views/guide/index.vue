...
const quickstart = [
  { title: '登录系统', desc: '使用已有账号登录；没有账号时可在登录页点击「立即注册」自助创建医生账号。演示环境可在登录页一键填入演示账号。' },
  { title: '上传眼底影像', desc: '进入「筛查上传」，填写患者信息（选填）并选择一张或多张眼底图；可点击缩略图放大预览，确认无误后点击「开始筛查」。' },
...
const features = [
  { icon: 'upload', name: '筛查上传', desc: '选择眼底图后先预览确认，再点击「开始筛查」逐张独立推理；单张失败不影响其它影像，可单独重试。' },
  ...
  { icon: 'clock', name: '操作日志', desc: '记录登录、注册、上传、删除、导出、复核与账号变更等关键动作，含操作人、耗时与 IP。' }
]
...
const faqs = [
  {
    q: '如何注册新账号？',
    a: '在登录页点击「立即注册」，填写用户名、真实姓名与密码即可。注册账号固定为医生角色，仅可查看本人上传的筛查记录；管理员账号需由既有管理员在「系统管理 → 用户管理」中创建。'
  },
  {
    q: '为什么我看不到某条筛查记录？',
